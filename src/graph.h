...
public:
	...
	void AddDirectionalEdge(const NodeID from, const NodeID to, const EdgeData& edge) {
	...
	void AddNonDirectionalEdge(const NodeID from, const NodeID to, const EdgeData& edge) {
		this->edges.push_back({ .from = from, .to = to, .data = edge });
	...
	bool FindNodeID(const NodeData& node, NodeID& outNodeID) {
	...
	bool FindEdgeBetweenNodes(NodeID from, NodeID to, EdgeData& outEdge) {
